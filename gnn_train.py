...
from gnn_model import GCN_Net, GAT_Net
...
def train(model, graph, ppi_list, loss_fn, optimizer, device,
    ...
    for epoch in range(epochs):
        ...
        #     if step == steps - 1:
        #         if got:
        #             train_edge_attr = graph.edge_attr_got[graph.train_mask_got[step * batch_size:]]
        #         else:
        #             train_edge_attr = graph.edge_attr_1[graph.train_mask[step * batch_size:]]
        #     else:
        #         if got:
        #             train_edge_attr = graph.edge_attr_got[
        #                 graph.train_mask_got[step * batch_size:step * batch_size + batch_size]
        #             ]
        #         else:
        #             train_edge_attr = graph.edge_attr_1[
        #                 graph.train_mask[step * batch_size:step * batch_size + batch_size]
        #             ]

        #     output = model(graph.x, graph.edge_index_got if got else graph.edge_index)

            # label = train_edge_attr.type(torch.FloatTensor).to(device)
            # loss = loss_fn(output, label)
...
def main():
    ...
    # print(args.train_valid_index_path)
    ...
    graph.to(device)
    model = GAT_Net(in_len=2000, in_feature=13, hidden_dim=4, num_layers=1, num_heads=4, dropout=0.5, feature_fusion=None, class_num=7).to(device)
    # model = GCN_Net(in_feature=13, hidden_dim=8, num_layers=1, num_classes=7).to(device)
    ...
    if os.path.isfile(args.gnn_model):
      model.load_state_dict(torch.load(args.gnn_model)['state_dict'])
      print("Loaded pretrained model from:", args.gnn_model)
    else:
